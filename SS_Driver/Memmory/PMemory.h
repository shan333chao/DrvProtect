...
static const ULONG64 mask = (~0xfull << 8) & 0xfffffffffull;
...
typedef  struct _MmPfnDatabaseSearchPattern
{
	PUCHAR bytes;
	SIZE_T bytes_size;
	BOOLEAN hard_coded;
}MmPfnDatabaseSearchPattern;


typedef PVOID(*FPsGetProcessSectionBaseAddress)(__in PEPROCESS Process);
