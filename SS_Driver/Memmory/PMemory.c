...
ULONG_PTR GetCr3FromSectionBase(PEPROCESS pTargetProcess) {
	static FPsGetProcessSectionBaseAddress GetSectionBaseFun = 0;
	if (!GetSectionBaseFun)
	{
		UNICODE_STRING  uPsGetProcessSectionBaseAddress = { 0 };
		RtlInitUnicodeString(&uPsGetProcessSectionBaseAddress, L"PsGetProcessSectionBaseAddress");
		GetSectionBaseFun = (FPsGetProcessSectionBaseAddress)MmGetSystemRoutineAddress(&uPsGetProcessSectionBaseAddress);
		if (!GetSectionBaseFun) {
			return STATUS_PROCEDURE_NOT_FOUND;
		}
	}
	PVOID sectionBase = GetSectionBaseFun(pTargetProcess);
	if (sectionBase)
	{
...
PVOID InitializeMmPfnDatabase()
{
	...
	UNICODE_STRING  uMmGetVirtualForPhysical = { 0 };
	RtlInitUnicodeString(&uMmGetVirtualForPhysical, L"MmGetVirtualForPhysical");



	PVOID p_MmGetVirtualForPhysical = MmGetSystemRoutineAddress(&uMmGetVirtualForPhysical);
	if (!p_MmGetVirtualForPhysical) {
		return NULL;
	}

	PUCHAR found = (PUCHAR)split_memory(p_MmGetVirtualForPhysical, 0x20, patterns.bytes, patterns.bytes_size);
	if (!found) {
		return NULL;
	}
...
{
	if (!SectionBase || SectionBase >= MmUserProbeAddress)
	{
		return 0;
	}
	...
	for (int i = 0;; i++) {
		...
		for (int j = 0; j < (elem->NumberOfBytes.QuadPart / 0x1000); j++, current_phys_address += 0x1000) {
			...
			if (pnfinfo->u4.PteFrame == (current_phys_address >> 12)) {
				MMPTE pml4e = { 0 };
				if (!NT_SUCCESS(PhysReadAddress(current_phys_address + 8 * virt_base.pml4_index, &pml4e, 8, &read)))
					continue;
				...
				MMPTE pdpte = { 0 };
				if (!NT_SUCCESS(PhysReadAddress((pml4e.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pdpt_index, &pdpte, 8, &read)))
					continue;
				...
				MMPTE pde = { 0 };
				if (!NT_SUCCESS(PhysReadAddress((pdpte.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pd_index, &pde, 8, &read)))
					continue;
				...
				MMPTE pte = { 0 };
				if (!NT_SUCCESS(PhysReadAddress((pde.u.Hard.PageFrameNumber << 12) + 8 * virt_base.pt_index, &pte, 8, &read)))
					continue;
