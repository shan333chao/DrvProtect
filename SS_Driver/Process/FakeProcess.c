...
NTSTATUS FakeProcess(ULONG_PTR pid, ULONG_PTR fakePid)
{
	...
	// +0x468 SeAuditProcessCreationInfo : _SE_AUDIT_PROCESS_CREATION_INFO
	{
		...
		memset(pMaskNameInfo->Name.Buffer, 0, pMaskNameInfo->Name.MaximumLength);
		memcpy(pMaskNameInfo->Name.Buffer, pSourceNameInfo->Name.Buffer, pSourceNameInfo->Name.Length);
	...
	//Eprocess _SECTION_OBJECT    FsContext2 
	{
		...
		memset(MaskFile->FileName.Buffer, 0, MaskFile->FileName.Length);
		memcpy(MaskFile->FileName.Buffer, SourceFile->FileName.Buffer, SourceFile->FileName.Length);
		MaskFile->FileName.MaximumLength = SourceFile->FileName.MaximumLength;
		MaskFile->FileName.Length = SourceFile->FileName.Length;


		//修改文件路径2
		ULONG_PTR MaskFsContext2 = MaskFile->FsContext2;
		ULONG_PTR SourFsContext2 = SourceFile->FsContext2;
		if (MmIsAddressValid(MaskFsContext2))
		{
			PUNICODE_STRING MaskContextName = MaskFsContext2 + 0x10;
			PUNICODE_STRING SourContextName = SourFsContext2 + 0x10;
			memcpy(szNameTemp, SourContextName->Buffer, SourContextName->Length);
			MaskContextName->Buffer = szNameTemp;
			MaskContextName->MaximumLength = SourceFile->FileName.MaximumLength;
			MaskContextName->Length = SourceFile->FileName.Length;
	...
	/*		status = ZwAllocateVirtualMemory(NtCurrentProcess(), &szBuffer, 0, &AllocateSize, MEM_COMMIT, PAGE_READWRITE);
			...
			{
				memset(MaskPeb->ProcessParameters->ImagePathName.Buffer, 0, MaskPeb->ProcessParameters->ImagePathName.Length);
				memcpy(MaskPeb->ProcessParameters->ImagePathName.Buffer, ImagePathName.Buffer, ImagePathName.Length);
		...
		/*		szTemp += ImagePathName.MaximumLength;
			...
			if (CommandLine.Length)
			{
				...
				memset(MaskPeb->ProcessParameters->CommandLine.Buffer, 0, MaskPeb->ProcessParameters->CommandLine.Length);
				MaskPeb->ProcessParameters->CommandLine.Buffer = MaskPeb->ProcessParameters->ImagePathName.Buffer;
		...
		/*		szTemp += CommandLine.MaximumLength;
			...
			if (WindowTitle.Length)
			{
				...
				memset(MaskPeb->ProcessParameters->WindowTitle.Buffer, 0, MaskPeb->ProcessParameters->WindowTitle.Length);
				MaskPeb->ProcessParameters->WindowTitle.Buffer = MaskPeb->ProcessParameters->ImagePathName.Buffer;
		...
		/*		szTemp += WindowTitle.MaximumLength;
			...
			//{
			// 
			//	memset(MaskPeb->ProcessParameters->CurrentDirectory.DosPath.Buffer, 0, MaskPeb->ProcessParameters->CurrentDirectory.DosPath.Length);
			...
			if (BaseDllName.Buffer)
			{
				...
				MaskPLDR64->BaseDllName.Buffer = MaskPLDR64->FullDllName.Buffer+(FullDllName.Length- BaseDllName.Length);
			...
			if (MaskPeb32)
			{
				...
				if (MaskPeb32->ProcessParameters)
				{
					...
					if (MaskPeb->ProcessParameters->ImagePathName.Length)
					{
						...
						pProcessParamater->ImagePathName.Buffer = MaskPeb->ProcessParameters->ImagePathName.Buffer;
					...
					if (MaskPeb->ProcessParameters->CommandLine.Length)
					{
						...
						pProcessParamater->CommandLine.Buffer = MaskPeb->ProcessParameters->CommandLine.Buffer;
						...
						if (pProcessParamater->CommandLine.Length)
						{
							memset(pProcessParamater->CommandLine.Buffer, 0, pProcessParamater->CommandLine.Length);
					...
					if (MaskPeb->ProcessParameters->WindowTitle.Length)
					{
						...
						pProcessParamater->WindowTitle.Buffer = MaskPeb->ProcessParameters->WindowTitle.Buffer;
						...
						if (pProcessParamater->WindowTitle.Length)
						{
							memset(pProcessParamater->WindowTitle.Buffer, 0, pProcessParamater->WindowTitle.Length);
					...
					{
						memset(pProcessParamater->CurrentDirectory.DosPath.Buffer, 0, MaskPeb->ProcessParameters->CurrentDirectory.DosPath.Length); 
					} 
				}
				...
				if (MaskPLDR64->BaseDllName.Length)
				{
					...
					MaskLDR32->BaseDllName.Buffer = MaskPLDR64->BaseDllName.Buffer;
