...
namespace k_utils
{
	...
	unsigned long long get_module_address(const char* name, unsigned long* size)
	{
		...
		ZwQuerySystemInformation(system_module_information, &length, 0, &length);
		...
		NTSTATUS status = ZwQuerySystemInformation(system_module_information, system_modules, length, 0);
