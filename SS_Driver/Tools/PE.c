...
{
    if ((PUCHAR)ptr < (PUCHAR)lpHeader)
        return 0;
    ULONG dwRva = (ULONG)((PUCHAR)ptr - (PUCHAR)lpHeader);
    IMAGE_DOS_HEADER* pdh = (IMAGE_DOS_HEADER*)lpHeader;
    if (pdh->e_magic != IMAGE_DOS_SIGNATURE)
        return 0;
    IMAGE_NT_HEADERS* pnth = (IMAGE_NT_HEADERS*)((PUCHAR)lpHeader + pdh->e_lfanew);
    ...
    return (PVOID)((PUCHAR)lpHeader + psh[section].VirtualAddress);
}
...
ULONG GetExportOffset(const PUCHAR FileData, ULONG FileSize, const PCHAR ExportName)
{
    ...
    if (pdh->e_magic != IMAGE_DOS_SIGNATURE)
    {
        Log("[SSS] Invalid IMAGE_DOS_SIGNATURE!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    if (pnth->Signature != IMAGE_NT_SIGNATURE)
    {
        Log("[SSS] Invalid IMAGE_NT_SIGNATURE!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    {
        Log("[SSS] Invalid Export Directory!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    {
        Log("[SSS] Invalid Export Directory Contents!\r\n");
        return PE_ERROR_VALUE;
    }
    ...
    for (ULONG i = 0; i < NumberOfNames; i++)
    {
        ...
        if (CurrentNameOffset == PE_ERROR_VALUE)
            continue;
        PCHAR CurrentName = (PCHAR)(FileData + CurrentNameOffset);
    ...
    {
        Log("[SSS] Export %s not found in export table!\r\n", ExportName);
    }
